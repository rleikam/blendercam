import bpy
from bpy.props import BoolProperty, FloatProperty, FloatVectorProperty, EnumProperty, IntProperty
from .callback import updateMachine
...
class MachineSettings(bpy.types.PropertyGroup):
    ...
    use_position_definitions: BoolProperty(name="Use position definitions",
                                                     description="Define own positions for op start, "
                                                                 "toolchange, ending position",
                                                     default=False)
    starting_position: FloatVectorProperty(name='Start position', default=(0, 0, 0), unit='LENGTH',
                                                     precision=PRECISION, subtype="XYZ", update=updateMachine)
    mtc_position: FloatVectorProperty(name='MTC position', default=(0, 0, 0), unit='LENGTH',
                                                precision=PRECISION, subtype="XYZ", update=updateMachine)
    ending_position: FloatVectorProperty(name='End position', default=(0, 0, 0), unit='LENGTH',
                                                   precision=PRECISION, subtype="XYZ", update=updateMachine)

    working_area: FloatVectorProperty(name='Work Area', default=(0.500, 0.500, 0.100), unit='LENGTH',
                                                precision=PRECISION, subtype="XYZ", update=updateMachine)
    feedrate_min: FloatProperty(name="Feedrate minimum /min", default=0.0, min=0.00001, max=320000,
                                          precision=PRECISION, unit='LENGTH')
    feedrate_max: FloatProperty(name="Feedrate maximum /min", default=2, min=0.00001, max=320000,
                                          precision=PRECISION, unit='LENGTH')
    feedrate_default: FloatProperty(name="Feedrate default /min", default=1.5, min=0.00001, max=320000,
                                              precision=PRECISION, unit='LENGTH')
    hourly_rate: FloatProperty(name="Price per hour", default=100, min=0.005, precision=2)

    # UNSUPPORTED:

    spindle_min: FloatProperty(name="Spindle speed minimum RPM", default=5000, min=0.00001, max=320000,
                                         precision=1)
    spindle_max: FloatProperty(name="Spindle speed maximum RPM", default=30000, min=0.00001, max=320000,
                                         precision=1)
    spindle_default: FloatProperty(name="Spindle speed default RPM", default=15000, min=0.00001, max=320000,
                                             precision=1)
    spindle_start_time: FloatProperty(name="Spindle start delay seconds",
    ...
    eval_splitting: BoolProperty(name="Split files",
    ...
    collet_size: FloatProperty(name="#Collet size", description="Collet size for collision detection",
