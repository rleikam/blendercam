...
#	output_image_name: bpy.props.StringProperty(name='Image target', description='image output name')
	...
	widthmm: IntProperty(name="Desired width in mm", default=200, min=5, max=4000)
	heightmm: IntProperty(name="Desired height in mm", default=150, min=5, max=4000)
	thicknessmm: IntProperty(name="Thickness in mm", default=15, min=5, max=100)

	justifyx: EnumProperty(name="X",items=[('1', 'Left','', 0),('-0.5', 'Centered','', 1),('-1', 'Right','', 2)],default='-1')
	justifyy: EnumProperty(name="Y",items=[('1', 'Bottom','', 0),('-0.5', 'Centered','', 2),('-1', 'Top','', 1),],default='-1')
	justifyz: EnumProperty(name="Z",items=[('-1', 'Below 0','', 0),('-0.5', 'Centered','', 2),('1', 'Above 0','', 1),],default='-1')
	...
	silhouette_exponent: IntProperty(name="Silhouette square exponent", description="If lower, true depht distances between objects will be more visibe in the relief", default=3, min=0, max=5)
	...
	vcycle_iterations: IntProperty(name="V-cycle iterations",description="set up higher for plananr constraint", default=2, min=1, max=128)
